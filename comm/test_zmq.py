from comm.tensor_socket import CommCS, PORT
import time
import torch
...
def server_process(server_ip):
    ...
    """服务器进程"""
    print(f"[SERVER] Starting server on {server_ip}:{PORT}...")
...
def client_process(server_ip):
    ...
    # 客户端连接到本地服务器
    client = CommCS(server_ip, is_server=False)
...
if __name__ == "__main__":
    ...
    if is_server:
        server_process(server_ip)
    else:
        client_process(server_ip)
